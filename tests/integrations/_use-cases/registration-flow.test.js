...
describe("Use case: Registration Flow (All successful)", () => {
    ...
    const createUserResponse = await fetch(
      "http://localhost:3000/api/v1/users",
      {
        ...
        body: JSON.stringify({
          ...
          password: "RegistrationFlowPasswprd",
        }),
      },
    );
  ...
  test("Login", async () => {
    ...
    const acreateSessionResponseBody = await createSessionResponse.json();

    expect(acreateSessionResponseBody.user_id).toBe(createUserResponseBody.id);
  });
});
